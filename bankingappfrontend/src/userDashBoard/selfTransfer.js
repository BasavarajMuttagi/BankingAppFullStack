...
function SelfTransfer() {
    ...
    const handleMySelfTransfer = (e) =>{
            e.preventDefault()
            const userName = localStorage.getItem('userName')
             axios.post("http://localhost:8080/api/selftransfer",{userName,bankAbbreviationSender,bankAbbreviationReceiver,amount}).then(resp=>{
              if(resp.status === 200){
    ...
    let dropDown = Object.values(bankObj).map((eachBank)=>{return(<option value={eachBank} key={eachBank}>{eachBank}</option>)})
      ...
      <div className="col-sm-9 col-md-7 col-lg-5 mx-auto">
        <div className="card border-0 shadow rounded-3 my-5">
          <div className="card-body p-4 p-sm-5">
            <h5 className="card-title text-center mb-5 fw-light fs-5">Self Transfer</h5>
            <form onSubmit={handleMySelfTransfer} >
              ...
              <div className="d-grid">
                <button className="btn btn-primary btn-login text-uppercase fw-bold" type="submit">Self Transfer</button>
              </div>
