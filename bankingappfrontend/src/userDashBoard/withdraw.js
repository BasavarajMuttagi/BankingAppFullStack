...
function WithDraw() {
    ...
    const handleMyWithDraw = (e) =>{
            e.preventDefault()
            const userName = localStorage.getItem('userName')
             axios.post("http://localhost:8080/api/withdraw",{userName,bankAbbreviation,amount}).then(resp=>{
              if(resp.status === 200){
      ...
      <div className="col-sm-9 col-md-7 col-lg-5 mx-auto">
        <div className="card border-0 shadow rounded-3 my-5">
          <div className="card-body p-4 p-sm-5">
            <h5 className="card-title text-center mb-5 fw-light fs-5">Withdraw</h5>
            <form onSubmit={handleMyWithDraw} >
